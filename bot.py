...
def pergessle(input):
    ...
    if input.lower().startswith("pergessle --help")  or input.lower().startswith("pergessle help"):
        ...
        msg += " --song [SEARCH_QUERY]: Returns 3 songs from songs with [SEARCH_QUERY] in them.\n"
        msg += " --artist [SEARCH_QUERY]: Returns 3 songs from artists with [SEARCH_QUERY] in them.\n"
        msg += " --amount [N_SONGS]: Returns [N_SONGS] songs from full song list. \n"
        msg += " --artist_popularity [VAL 0-100] \n"
        msg += " --song_popularity [VAL 0-100] \n"
        msg += " --release_before [YEAR] \n"
        msg += " --release_after [YEAR] \n"
        msg += " --no-replace: Sets flag to use no replacement, e.g. we can't get the same song twice.\n"
        msg += " --quiet: Hides extra msg information except the songs.\n"
        msg += " --include-uri: Includes a spotify uri to the song if exists.\n"
        msg += " --include-id: Includes the local id to the song.\n"
    ...
    pers_band = ["per gessle", "gyllene tider"]
    if include_id:
        res = [f"{r[1]}: {r[2]}, {r[0]}" for r in res]
    else:
        res = [f"{r[1]}: {r[2]}" if r[1].lower() not in pers_band else f"{r[1]}: {r[2]} :minidisc:" for r in res]
